...
import {
  ...
  LogOut, GraduationCap, Sun, Moon, ExternalLink,
} from "lucide-react";
    ...
    >
      ...
      <div className="shrink-0 flex items-center min-w-[120px]">
        {logoUrl ? (
          ...
          <span className="font-semibold text-sm tracking-tight gradient-text-portfolio" style={{ fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div className="shrink-0 flex items-center gap-2">
        ...
        <div className="h-7 w-px" style={{ background: "hsl(0,0%,20%)" }} />
        <div
        ...
        <span className="text-xs hidden sm:block" style={{ color: "hsl(0,0%,60%)" }}>{user?.name}</span>
        <button
          className="h-7 w-7 rounded-lg flex items-center justify-center transition-colors duration-150"
...
function TopNavItem({ item }) {
  ...
  return (
    <NavLink
      to={path}
      className="sidebar-link flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium transition-all duration-150"
      style={{ color: isActive ? "hsl(var(--primary))" : "hsl(0,0%,60%)", background: isActive ? "hsl(var(--primary) / 0.1)" : "transparent" }}
